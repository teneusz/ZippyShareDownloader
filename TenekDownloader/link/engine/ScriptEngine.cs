...
namespace TenekDownloader.link.engine
{
	/// <summary>
	...
	public sealed class ScriptEngine : IDisposable
	{
		...
		public Version Version
		{
			get
			{
				if (_version == null)
				{
					var major = GetProperty(SCRIPTPROP_MAJORVERSION, 0);
					var minor = GetProperty(SCRIPTPROP_MINORVERSION, 0);
					var revision = GetProperty(SCRIPTPROP_BUILDNUMBER, 0);
		...
		public string Name
		{
			get
			{
				if (_name == null) _name = GetProperty(SCRIPTPROP_NAME, string.Empty);
				return _name;
			}
		}
		...
		public static Version GetVersion(string language)
		{
			...
			var scriptEngine = Activator.CreateInstance(engine) as IActiveScript;
			if (scriptEngine == null)
				return null;

			var scriptProperty = scriptEngine as IActiveScriptProperty;
			if (scriptProperty == null)
				return new Version(1, 0, 0, 0);

			var major = GetProperty(scriptProperty, SCRIPTPROP_MAJORVERSION, 0);
			var minor = GetProperty(scriptProperty, SCRIPTPROP_MINORVERSION, 0);
			var revision = GetProperty(scriptProperty, SCRIPTPROP_BUILDNUMBER, 0);
			var version = new Version(major, minor, Environment.OSVersion.Version.Build, revision);
		...
		{
			value = null;
			var property = _engine as IActiveScriptProperty;
			if (property == null)
				return false;
		...
		private object Parse(string text, bool expression)
		{
			...
			if (expression) flags |= ScriptText.IsExpression;
			...
			try
			{
				...
				if (_parse32 != null)
				{
					if (expression) text = varName + "=" + text;
		...
		private interface IActiveScript
		{
			...
			[PreserveSig]
			int InterruptScriptThread(uint thread, out EXCEPINFO exceptionInfo,
				uint flags);
		...
		private interface IActiveScriptSite
		{
			...
			[PreserveSig]
			int OnScriptTerminate(object result, EXCEPINFO exceptionInfo);
		...
		{
			[PreserveSig]
			int GetExceptionInfo(out EXCEPINFO exceptionInfo);
		...
		private interface IActiveScriptParse32
		{
			...
			[PreserveSig]
			int AddScriptlet(string defaultName, string code, string itemName, string subItemName, string eventName,
			...
			[PreserveSig]
			int ParseScriptText(string code, string itemName, IntPtr context, string delimiter, int sourceContextCookie,
				uint startingLineNumber, ScriptText flags, out object result,
		...
		private interface IActiveScriptParse64
		{
			...
			[PreserveSig]
			int ParseScriptText(string code, string itemName, IntPtr context, string delimiter,
				long sourceContextCookie, uint startingLineNumber, ScriptText flags, out object result,
				out EXCEPINFO exceptionInfo);
		}
		...
		internal class ScriptSite : IActiveScriptSite
		{
			...
			int IActiveScriptSite.OnScriptTerminate(object result,
				EXCEPINFO exceptionInfo)
			{
				return 0;
			}
			...
			int IActiveScriptSite.OnScriptError(IActiveScriptError scriptError)
			{
				...
				LastException = new ScriptException(string.Format(message, exceptionInfo.scode,
					exceptionInfo.bstrSource, exceptionInfo.bstrDescription, lineNumber, characterPosition,
					sourceLine));
