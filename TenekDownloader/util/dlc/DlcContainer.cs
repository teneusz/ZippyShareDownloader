...
/// <summary>
/// https://github.com/AyrA/undlc
/// </summary>
namespace TenekDownloader.util.dlc
{
	/// <summary>
	///     DLC file Decrypter
	/// </summary>
	public class DlcContainer
	{
		...
		private const string GETKEY_URL =
		...
		private readonly string DlcContent;

		/// <summary>
		///     The real IV to decrypt <see cref="SourceData" />
		/// </summary>
		private readonly byte[] RealIV;

		/// <summary>
		///     The real key to decrypt <see cref="SourceData" />
		/// </summary>
		private readonly byte[] RealKey;
		...
		/// <param name="Comment">Package Comment</param>
		/// <returns>DLC string</returns>
		public static string CreateDlc(IEnumerable<DlcFile> FileLinks, DlcHeader Header, string PackageName,
			string Comment)
		{
			if (FileLinks.Count() > 0)
			{
				...
				var Encrypted = Tools.B64(AesEncrypt(Tools.ASC(Tools.B64(Tools.ASC(XML))), Tools.ASC(Tools.B2S(RealKey)),
					Tools.ASC(Tools.B2S(RealKey))));
		...
		private static string BuildXML(IEnumerable<DlcFile> FileLinks, DlcHeader Header, string PackageName,
			string Comment)
		{
			if (string.IsNullOrEmpty(PackageName)) Comment = "unknown";
			if (string.IsNullOrEmpty(Comment)) Comment = PackageName;
			//Build XML Document
			var d = new XmlDocument();
